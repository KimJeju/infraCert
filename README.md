...
- Windows 룰은 이 개발기(Win11)에서 `transport/local.LocalConnection` 으로 64개 전부 실행 검증했다(예외 0). Oracle 은 WSL docker(oracle-free 26ai)로 26룰 실검증, Cisco 는 가짜 IOS SSH 서버로 전송 계층까지 — 실장비 대기.
