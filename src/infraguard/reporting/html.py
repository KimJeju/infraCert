    ...
    for h in scan.hosts:
        for r in h.results:
            rows.append(
                ...
                f"<td>{_esc(_diff.LABEL_KO[c] if (c := d.get((h.hostname, r.rule_id), (None, None))[1]) else '')}</td></tr>"
            )
...
<div class="cards">{cards}</div>{fix_cards}
<table><thead><tr><th>호스트</th><th>항목코드</th><th>점검항목</th><th>중요도</th>
<th>진단결과</th><th>판정근거</th><th>점검내용</th><th>조치방법</th><th>판단기준</th><th>판정추적</th><th>이전결과</th><th>변화</th></tr></thead>
