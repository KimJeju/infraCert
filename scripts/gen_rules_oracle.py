...
RULES: list[dict] = [
    ...
    rule("D-04", "데이터베이스 관리자 권한을 꼭 필요한 계정 및 그룹에 대해서만 허용", "상", ACC,
         [c("q", sq("SELECT grantee FROM dba_role_privs WHERE granted_role='DBA' AND grantee NOT IN ('SYS','SYSTEM');"))],
    ...
    rule("D-06", "DB 사용자 계정을 개별적으로 부여하여 사용", "중", ACC,
         [c("q", sq(f"SELECT username||':'||account_status FROM dba_users WHERE username NOT IN ({SYS_SCHEMAS}) ORDER BY 1;"))],
    ...
    rule("D-11", "DBA 이외의 인가되지 않은 사용자가 시스템 테이블에 접근할 수 없도록 설정", "상", PERM,
         [c("q", sq("SELECT value FROM v$parameter WHERE name='O7_DICTIONARY_ACCESSIBILITY';")),
         ...
         [{"when": {"q": {"regex": r"(?i)TRUE"}}, "then": "VULN"}, {"when": {"pub": {"gt": 0}}, "then": "VULN"},
          {"when": {"q": {"regex": r"(?i)FALSE"}}, "then": "GOOD"}, {"else": "MANUAL"}]),
    ...
    rule("D-14", "데이터베이스의 주요 설정 파일, 비밀번호 파일 등과 같은 주요 파일들의 접근 권한 설정", "중", PERM,
         [c("w", "find \"$ORACLE_HOME/network/admin\" \"$ORACLE_HOME/dbs\" -type f \\( -name '*.ora' -o -name 'orapw*' \\) -perm -o+w 2>/dev/null"),
    ...
    rule("D-20", "인가되지 않은 Object Owner의 제한", "하", PERM,
         [c("q", sq(f"SELECT DISTINCT owner FROM dba_objects WHERE owner NOT IN ({SYS_SCHEMAS}) ORDER BY 1;"))],
         [{"when": {"q": {"absent": True}}, "then": "GOOD"}, {"else": "MANUAL"}], note="시스템 스키마 외 오브젝트 소유자는 인가 여부 판단"),
    rule("D-21", "인가되지 않은 GRANT OPTION 사용 제한", "중", PERM,
         [c("q", sq("SELECT grantee||':'||owner||'.'||table_name FROM dba_tab_privs WHERE grantable='YES' AND grantee NOT IN ('SYS','SYSTEM') AND grantee NOT IN (SELECT role FROM dba_roles) AND owner NOT IN ('SYS','SYSTEM','XDB','MDSYS','CTXSYS','ORDSYS','WMSYS');"))],
         [{"when": {"q": {"absent": True}}, "then": "GOOD"}, {"else": "VULN"}], note="롤이 아닌 일반 계정에 WITH GRANT OPTION 이 있으면 취약"),
